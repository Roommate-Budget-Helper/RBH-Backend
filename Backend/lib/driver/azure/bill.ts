...
const createUser2Bill = async (
    ...
    for (let i = 0; i < roommates.length; i++) {
        ...
        if (sharePlanid == -1) {
            // tslint:disable-next-line: no-floating-promises
            runQueryGetOne(`INSERT INTO dbo.shareRatioId (sharePlansid, userName, ratio) VALUES (${planId}, \'${roommates[i]}\', ${proportion[i]})`);
        }else{
            runQueryGetOne(`INSERT INTO dbo.shareRatioId (sharePlansid, userName, ratio) VALUES (${sharePlanid}, \'${roommates[i]}\', ${proportion[i]})`);
